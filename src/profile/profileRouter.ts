...
/** Latest AI technique score as 0–100 for aggregates (legacy analyses scaled from 0–10). */
function scorePercentFromTechniqueMetrics(metrics: unknown): number | null {
  if (!metrics || typeof metrics !== "object") return null;
  const m = metrics as Record<string, unknown>;
  const ai = m.ai_analysis as Record<string, unknown> | undefined;
  return storedAiScoreToPercent(ai);
}
...
router.get("/rating-by-category", async (req, res) => {
  try {
    ...
    let overallTw = 0,
      ...
      overallLw = 0,
      overallLwN = 0;
    let breakdownTwTechnique = 0,
      breakdownTwOutcome = 0,
      breakdownTwTactics = 0,
      breakdownTwN = 0;

    for (const row of rows) {
      const score = scorePercentFromTechniqueMetrics(row.metrics);
      if (score == null) continue;
      const ai = (row.metrics as Record<string, unknown> | null | undefined)
        ?.ai_analysis as Record<string, unknown> | undefined;
      const breakdownVals = storedAiBreakdownToPercent(ai);
      ...
      if (inThis) {
        ...
        overallTwN += 1;
        if (
          breakdownVals.technique != null &&
          breakdownVals.outcome != null &&
          breakdownVals.tactics != null
        ) {
          breakdownTwTechnique += breakdownVals.technique;
          breakdownTwOutcome += breakdownVals.outcome;
          breakdownTwTactics += breakdownVals.tactics;
          breakdownTwN += 1;
        }
      } else if (inLast) {
        overallLw += score;
    ...
    return res.json({
      ...
      breakdownOverall: {
        technique: breakdownTwN > 0 ? round1(breakdownTwTechnique / breakdownTwN) : null,
        outcome: breakdownTwN > 0 ? round1(breakdownTwOutcome / breakdownTwN) : null,
        tactics: breakdownTwN > 0 ? round1(breakdownTwTactics / breakdownTwN) : null,
      },
    });
  } catch (e: any) {
...
router.get("/coach-students", async (req, res) => {
  try {
    ...
    for (const row of scoreRows) {
      ...
      const scorePct = storedAiScoreToPercent(ai);
      if (scorePct == null || !Number.isFinite(scorePct)) continue;
